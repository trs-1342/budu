...
type Country = {
  ...
  name: string;
  dial: string; // +90
  mask?: (digits: string) => string; // görüntü formatı
  placeholder?: string;
};

const COUNTRIES: Country[] = [
  // — Özel maskeli örnekler (senden)
  {
    code: "TR",
    name: "Türkiye",
    dial: "+90",
  ...
  // — Aşağıdan itibaren: tüm ülke/bağımlılıklar (alfabetik, ISO-3166-1 alpha-2)
...
type LoginRes = {
  token: string;
  user: { id: number; email: string; username: string; phone?: string };
};
...
export default function Auth() {
  ...
  const phoneMasked = useMemo(() => {
    if (!phoneRaw) return "";
    const only = phoneRaw.replace(/\D/g, "");
    return country.mask ? country.mask(only) : only;
  }, [phoneRaw, country]);

  useEffect(() => {
    // zaten girişliyse yönlendir
    if (getToken()) nav("/account", { replace: true });
  }, [nav]);
  ...
  return (
    <div className="auth-wrapper">
      <div className="auth-card reveal reveal--up">
        <div className="auth-toggle" role="tablist" aria-label="Giriş / Kayıt">
          <label className={`seg ${mode === "login" ? "active" : ""}`}>
            <input
              type="radio"
              name="auth-mode"
              checked={mode === "login"}
          ...
          <label className={`seg ${mode === "register" ? "active" : ""}`}>
            <input
              type="radio"
              name="auth-mode"
                ...
                <div className="phone-row">
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setCountry(next);
                      setPhoneRaw(""); // maske değişirse sıfırla
                    }}
                  ...
                  <div className="dial">{country.dial}</div>
                  <input
                    type="tel"
                    inputMode="numeric"
        ...
        <p className="fine">
          Devam ederek <a href="/terms">şartları</a> kabul etmiş olursun.
        </p>
      </div>
    </div>
  );
