...
const {
  ...
  DB_PASS = "password",
  DB_NAME = "BUDU",
  CLIENT_ORIGIN = "http://192.168.1.112:1001", // react dev server (vite)
} = process.env;
...
app.get("/", (req, res) => {
  ...
  res.send("server is running, go to <a href='http://192.168.1.112:1001'>client</a>");
});

app.listen(PORT, () => {
  console.log(`API listening on http://192.168.1.112:${PORT}`);
});
